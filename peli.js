...
//in case of error set width
if (screenWidth > 200) {
	//everything is ok
}else{screenWidth = 1024;}

//NETTISIVUN TAUSTAV�RI 	#8DC4C6
//HOHTEEN V�RI				#00F6FF
...
document.onkeyup=controllerup		//nappi yl�s
document.onkeydown=controllerdown	//nappi alas
	
context.fillStyle = "#A0A0CC";
context.fillRect(0,0,canvas.width, canvas.height);

var state = 0; //0 = loading screen, 1 = game is running, 2 = game won
var loadingStatus = 0; //how many images are loaded?
...
var copterVelX = 5*(canvas.width/1024);
var copterVelY = 1*(canvas.width/1024); //velocity towards the sky

var gravity = 1*(canvas.width/1024);


////--creating image objects and Loading the image files--

var kopteri1 = new Image();
kopteri1.src = "kopteri1.png";
kopteri1.onload = function() {loadingStatus++};
var kopteri2 = new Image();
kopteri2.src = "kopteri2.png";
kopteri2.onload = function() {loadingStatus++};
var kopteri3 = new Image();
kopteri3.src = "kopteri3.png";
kopteri3.onload = function() {loadingStatus++};
var kopteri4 = new Image();
kopteri4.src = "kopteri4.png";
kopteri4.onload = function() {loadingStatus++};


var uimariImg1 = new Image();
uimariImg1.src = "uimari1.png";
uimariImg1.onload = function() {loadingStatus++};
var uimariImg2 = new Image();
uimariImg2.src = "uimari2.png";
uimariImg2.onload = function() {loadingStatus++};
var uimariImg3 = new Image();
uimariImg3.src = "uimari3.png";
uimariImg3.onload = function() {loadingStatus++};
var uimariImg4 = new Image();
uimariImg4.src = "uimari4.png";
uimariImg4.onload = function() {loadingStatus++};
var uimariImgSaved = new Image();
uimariImgSaved.src = "uimariSaved.png";
uimariImgSaved.onload = function() {loadingStatus++};
...
vesiImg.onload = function() {loadingStatus++};
...
bgImage.onload = function() {loadingStatus++};

var rengasImg = new Image();
rengasImg.src = "rengas.png";
rengasImg.onload = function() {loadingStatus++};
...
var rengasState = []; //0 = ilmassa, 1 = vedess�, 2 = uimarilla

var winnerImg = new Image();
winnerImg.src = "winnertross.png";
winnerImg.onload = function() {loadingStatus++};

var palautus = false;
var keltanen = true;

// muutos() function is runed by every frame
function muutos(){	
	movements();
	animations();
	if (state == 0) loader(); //opening the Game
	if (state == 1) draw(); //game is running
	if(state == 2) stopGame(); //player has won
	if (document.getElementById("poster").width > canvas.width) newOrientation(); //is orientation drifferent?
	if (document.getElementById("poster").width < canvas.width) newOrientation(); //is orientation drifferent?
}
function loader() {
	////--loader() function draws the loading screen--
	//background
	...
	context.fillText("Loading a surprise... "+Math.round((loadingStatus*100)/13)+"%",canvas.height,canvas.height / 2);
	//is all images loaded?
	if (loadingStatus >= 13) {
		//recheck size for canvas
		if (document.getElementById("poster").width < 1024) {
			screenWidth = document.getElementById("poster").width;
			resize();
		}else{screenWidth = 1024;}
		state = 1
	};
...
function stopGame() {	
	...
	context.font=(30*gravity+"px Trebuchet MS");
	context.fillStyle = "#FFFFFF";
...
function reset() {
	...
	rengasX = [];
	rengasY = [];
	rengasState = []; //0 = ilmassa, 1 = vedess�, 2 = uimarilla
...
function resize() {
	...
	canvas.width = screenWidth;
	canvas.height = screenWidth * 0.25;
	//new size for all game metrics
	copterHeight = canvas.height * 0.2;
	copterWidth = copterHeight * 1.8582;
	flyingHeight = canvas.height*0.3;
	copterX = 0;
	copterY = flyingHeight + copterHeight * 0.5;
	copterVelX = 5*(canvas.width/1024);
	copterVelY = 1*(canvas.width/1024); //velocity towards the sky
	gravity = 1*(canvas.width/1024);
	uimariHeight = (2/3) * copterHeight;
	uimariWidth = uimariHeight * 0.8564;
	waterCenter = canvas.height * 0.8;
	waterLevel = waterCenter + uimariHeight* 0.25;
	uimariX = [getSwimmerX(1), getSwimmerX(2), getSwimmerX(3), getSwimmerX(4), getSwimmerX(5)];	//Swimmer x position
	waterWidth = canvas.width / 2;
	waterHeight = waterWidth * 0.66086;
	rengasWidth = 1.16761 * uimariWidth;
	rengasHeight = rengasWidth * 0.7705;
}

function newOrientation() {
	//if mobile user change orientation screen width change
	//resize
	screenWidth = document.getElementById("poster").width;
	resize();
	//restart the game
...
function moveCopter() {
	...
	copterVelY -= 0.25*gravity; //gravitation
	if (copterY > flyingHeight) {
		copterVelY += 0.5*gravity; //trust;
	}
	copterY -= copterVelY; //move copter position by speed;
}
function moveWater() {
	waterVelY -= 0.015*gravity; //gravitation
	if (waterLevel > waterCenter) {
		waterVelY += 0.03*gravity; //trust;
	}
	waterLevel -= waterVelY; //move copter position by speed;
	//lopeta vellominen
	if (waterLevel > canvas.height * 0.85 && waterVelY < 0) waterVelY = 0;
}
function moveRengas() {
